/*
    ...
    Simple flood forecasting simulation.
    This is the serial version used as the baseline.
*/
...
#define NUMCOLS 1000
#define PASSES 4
#define FLOODLEVEL 100
#define TIME 1.0
#define PONDINGDEPTH 5.0
...
{
    /* CHANGED: routing now uses the current water surface (terrain + water)
       instead of one fixed downhill target, so filled sinks can spill outward
       and flooded area can grow as storm duration increases. */
    (void)flowDirection;
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        /* CHANGED: replaced the old fixed 0.5f retention rule with an
           overflow threshold. Each cell now keeps up to PONDINGDEPTH of
           local water and only routes the excess, which slows flooding and
           makes spread depend on actual overflow instead of an arbitrary
           fraction every timestep. */
        float retainedWater = water[i];
        if (retainedWater > PONDINGDEPTH)
        {
            retainedWater = PONDINGDEPTH;
        }
...
int main(void)
{
    ...
    printf("The simulation is run with a 1000 x 1,000 grid\n");
    ...
    for (int i = 0; i < size; i++)
    {
        if (water[i] > maxWater) maxWater = water[i];
    }
    printf("Max water depth on any cell: %.2f mm\n", maxWater);

    int floodedCount = checkFlooding(water, floodedCells);

    /* Save terrain and flood maps for plotting. */
    FILE *terrainMap = fopen("terrain.dat", "w");
FILE *floodMap = fopen("flood.dat", "w");
for (int row = 0; row < NUMROWS; row++) {
    for (int col = 0; col < NUMCOLS; col++) {
        int i = row * NUMCOLS + col;
        fprintf(terrainMap, "%d %d %.2f\n", row, col, elevation[i]);
        fprintf(floodMap, "%d %d %d\n", row, col, floodedCells[i]);
    }
    fprintf(terrainMap, "\n");
    fprintf(floodMap, "\n");
}
...
fclose(floodMap);
    ...
    /* Save experiment results for graphs later. */
    FILE *rainfallData = fopen("rainfall_results.dat", "a");
if (rainfallData != NULL)
{
    fprintf(rainfallData, "%d %.1f %.1f\n", soilChoice, rainfall, (float)floodedCount / size * 100);
    fclose(rainfallData);
}

    FILE *durationData = fopen("duration_results.dat", "a");
    if (durationData != NULL)
    {
        fprintf(durationData, "%d %d %.1f\n", soilChoice, stormTime, (float)floodedCount / size * 100);
        fclose(durationData);
    }
